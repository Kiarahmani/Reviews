...
###### (Note by Kia Rahmani)
---
The chapter describes the notion of program invariants and how they can be used to define and prove correctness of some domains of programs.
- **Weakest Pre-condition:** For given F (formula) and S (program statement), wp(F,S) is defined to be the formula that if is satisfied in Inital state of S, after executing S we will end up in a state s' that satisfies F. (wp is a condition on initial state that is *enough* to satisfy F at the end ) 
  - Consequently, verification condition for a prgram trace is defined as: {F}S1; . . . ; Sn{G} : F ⇒ wp(G, S1; . . . ; Sn) 
- **Strongest Post-condition:** Similarly, sp is defined to be conditions on final states that if are satisfied, it means the execution has started from a state satisfying F.
...
The basic approach to find these mappings, is **symbolic forward propagation**. This algorithm iteraqtively tries to find the exact set of reachable states (states are represented abstractly by logical formulas), and update the potential mapping μ (that initially maps all non-zero locations to ⊥). The key idea is to pick a location -that still needs to be processed- and make sure that the *sp* of current mapping satisfies mapping of final location for a path or not. If so, we have reached the correct answer. Note that, this algorithm is not guaranteed to terminate. 

To address the above limitaion **Abstract interpretation** is introduced, that by over-approximating reachable states, can guarantee termination (Similar to what we already saw in bounded model checking and interpolation based *complete* model checking)
...
(Note that we might lose some information here, which would result in over-approximation, which is fine. For exampl, 0 ≤ i ∧ i ≤ 0 ∧ 0 ≤ n is mapped to: i=0)
